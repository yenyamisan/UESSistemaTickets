...
namespace UESTicketsProject.Controllers
{
    ...
    public class ReporterController : Controller
    {
        ...
        public ReporterController(IUsuarioRepository usuarioRepository, ITicketRepository ticketRepository, IEstatusesRepository estatusesRepository, IPrioridadRepository prioridadRepository, ITicketService ticketService)
        {
        ...
        public ActionResult Dashboard()
        {
            ...
            {
                Usuario = usuario,
                Tickets = _ticketRepository.TicketsReporterByUser(usuario.Id)
            };
            return View(model);
        }
        ...
        public ActionResult ListarTickets(string tipo)
        {
            ...
            switch (tipo)
            {
                case "Abiertos":
                    ...
                    model.Tickets = _ticketRepository.TicketsReporterByUser(usuario.Id).Where(x => x.EstadoActualId == 1)
                        .ToList();
                ...
                case "Completados":
                    ...
                    model.Tickets = _ticketRepository.TicketsReporterByUser(usuario.Id).Where(x => x.EstadoActualId > 3)
                        .ToList();
                    model.Estatus = "Completados";
                    break;
                case "Enprogreso":

                    model.Tickets = _ticketRepository.TicketsReporterByUser(usuario.Id).Where(x => x.EstadoActualId == 2)
                        .ToList();
